 ...
 * Author: Jay Sprenkle
 *
 * A program that reads an XmlCmd image document and creates a png image file.
 *
...
int main( int argc, char** argv )
{
   ...
   {
      // execution options
      ::XmlCmdImageToPng::Config::Settings config;
      ...
      for ( int i = 1; i < argc; ++i )
      {
         ...
         if ( arg == "-c" )
         {
            ...
            if ( ++i < argc )
            {
               ...
               try
               {
                  ::XmlCmdImageToPng::Config::Reader rdr( argv[i] );
                  config = rdr.Parse();
               }
