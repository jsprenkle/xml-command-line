...
namespace lsXml
{
   FileSystem::FileSystem( const ::lsXml::Config::Settings& config )
      : config( config )
      , doc( "http://www.XmlCommandLine.org/FileSystem/1.0" )
   {
   }
   ...
   void FileSystem::AppendContext( ::rapidxml::xml_node<>* Parent, int argc, char** argv )
   {
      ...
      doc.AppendAttribute( Create, "CreationDateTime", DateToString( time( 0 ) ) );

      doc.AppendChildNode( Create, "Notes", "Created by lsXML" );
      
      ::rapidxml::xml_node<>* Source = doc.AppendChildNode( Create, "Source" );
      doc.AppendAttribute( Source, "Name", "lsXml" );
            ...
            if ( next )
            {
               ...
               else
               {
...

                  {
                     ...
                     doc.AppendChildNode( element, "CreationDateTime", DateToString( _stat.st_ctime ) );
                     doc.AppendChildNode( element, "LastReadDateTime", DateToString( _stat.st_atime ) );
                     doc.AppendChildNode( element, "LastChangeDateTime", DateToString( _stat.st_mtime ) );
